...
function handleMousemove( event ) {
    ...
    var edgeTop = edgeTopBottom;
    var edgeLeft = edgeSide;
    var edgeBottom = ( viewportHeight - edgeTopBottom );
    var edgeRight = ( viewportWidth - edgeSide );
    ...
    function movedLeft() {
        return event.movementX < mousePositionX;
    }
    ...
    function movedUp() {
        return event.movementY < mousePositionY;
    }
    ...
    (function checkForWindowScroll() {
        ...
        if ( adjustWindowScroll() ) {
            ...
            timer = setTimeout( checkForWindowScroll, 5000 );
    ...
    function adjustWindowScroll() {
        ...
        if ( movedLeft() && canScrollLeft ) {

            // var intensity = ( ( edgeLeft - viewportX ) / edgeSide );

            nextScrollX = ( nextScrollX - maxStep );

        // Should we scroll right?
        } else if ( movedRight() && canScrollRight ) {

            // var intensity = ( ( viewportX - edgeRight ) / edgeSide );
        ...
        // Should we scroll up?
        if ( movedUp() && canScrollUp ) {

            // var intensity = ( ( edgeTop - viewportY ) / edgeTopBottom );

            nextScrollY = ( nextScrollY - maxStep );

        // Should we scroll down?
        } else if ( movedDown() && canScrollDown ) {

            // var intensity = ( ( viewportY - edgeBottom ) / edgeTopBottom );
...
function drawGridLines() {
    ...
    // edge.style.top = ( edgeTopBottom + "px" );
    // edge.style.bottom = ( edgeTopBottom + "px" );
    // edge.style.left = ( edgeSide + "px" );
    // edge.style.right = ( edgeSide + "px" );
